...
export function LoginPage() {
  ...
  const validateEmail = (val: string) => {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val)) return 'صيغة البريد الإلكتروني غير صحيحة'
    return undefined
  }
        ...
        <div className="w-full max-w-[440px]">
          ...
          <div className="rounded-2xl border border-white/10 bg-card/80 p-8 shadow-2xl backdrop-blur-xl">
            ...
            <div className="relative my-6">
              ...
              <div className="relative flex justify-center">
                <span className="bg-transparent px-4 text-xs text-muted-foreground">أو سجل الدخول بـ</span>
              </div>
            </div>
          ...
          <div className="mt-4 rounded-2xl border border-white/10 bg-card/60 p-5 text-center backdrop-blur-xl">
            <p className="text-sm text-muted-foreground">
              ليس لديك حساب؟{' '}
              <Link href="/?view=register" className="font-bold text-primary transition-colors hover:text-primary/80">
                أنشئ حساباً جديداً
              </Link>
            </p>
          </div>
